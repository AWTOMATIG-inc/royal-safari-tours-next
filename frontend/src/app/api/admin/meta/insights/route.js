...
export async function GET(request) {
  try {
    ...
    // 3. Retrieve Meta Ads Insights (Time-series data)
    const insightsData = await getInsights(startDate, endDate);
    ...
    console.error("[Meta Insights API Error]:", error.message || error);

    const status = error.status || 500;
    const code = error.code || "META_INSIGHTS_FETCH_FAILED";
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code,
          message: error.message || "Unable to retrieve Meta Ads historical insights.",
        },
      },
      { status }
    );
