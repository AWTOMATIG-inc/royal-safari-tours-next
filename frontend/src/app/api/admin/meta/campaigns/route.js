...
export async function GET(request) {
  try {
    ...
    // 3. Fetch Campaigns Data from Meta Service
    const campaignsData = await getCampaigns(startDate, endDate);
    ...
    console.error("[Meta Campaigns API Error]:", error.message || error);

    const status = error.status || 500;
    const code = error.code || "META_CAMPAIGNS_FETCH_FAILED";
    ...
    return NextResponse.json(
      {
        ...
        error: {
          code,
          message: error.message || "Unable to retrieve Meta Ads campaigns data.",
        },
      },
      { status }
    );
